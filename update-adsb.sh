...
git clone --quiet --depth 1 https://github.com/ADSB-One/adsb-update.git
cd adsb-update
...
; then
    ...
    echo "Please report this error to support@adsb.one or on discord."
    echo "--------------------"
fi
...
cat /boot/adsb-uuid
echo "#####################################"
sed -e 's$^$https://api.adsb.one/feeders/?feed=$' /boot/adsb-uuid
echo "#####################################"
